export default async function handler(req, res) {
  ...
  try {
    ...
    // 3. SEND EMAIL (don't fail registration if email fails)
    try {
      const emailResponse = await fetch(
        `${process.env.VERCEL_URL ? `https://${process.env.VERCEL_URL}` : 'http://localhost:3000'}/api/send-coach-email`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json'
      ...
      if (!emailResponse.ok) {
        console.error('Email sending failed, but registration succeeded');
      }
    } catch (emailError) {
